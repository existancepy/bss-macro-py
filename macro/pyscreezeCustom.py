...
def _locateAll_python(needleImage, haystackImage, grayscale=None, limit=None, region=None, step=1, confidence=None, var = 0):
    ...
    for y in range(haystackHeight): # start at the leftmost column
        for matchx in firstFindFunc(needleImageFirstRow, haystackImageData[y * haystackWidth:(y+1) * haystackWidth], step,var):
            foundMatch = True
            for searchy in range(1, needleHeight, step):
                haystackStart = (searchy + y) * haystackWidth + matchx
                needleRow = needleImageData[searchy * needleWidth:(searchy+1) * needleWidth]
                haystackRow = haystackImageData[haystackStart:haystackStart + needleWidth]
                #print(f'{needleRow}, {haystackRow}')
                if len(needleRow) != len(haystackRow):
                    break
                for a in range(len(haystackRow)):
                    needlePixel = needleRow[a]
                    haystackPixel = haystackRow[a]
                    foundMatch = pixelMatch(needlePixel, haystackPixel, var)
                    if not foundMatch:
                        break
...
def _kmp(needle, haystack, _dummy, var): # Knuth-Morris-Pratt search algorithm implementation (to be used by screen capture)
    """
    TODO
    """
    ...
    for c in haystack:
        while matchLen == len(needle) or \
              matchLen >= 0 and not pixelMatch(c, needle[matchLen], var):
